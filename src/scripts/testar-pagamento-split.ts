...
// Dados do pagador para uso em produção (obrigatórios)
const dadosPagador = {
...
async function testarPagamentoSplit() {
  ...
  try {
    ...
    const valorVendedor = valorTotal - taxaAplicacao; // R$ 90,00 (90%)
    ...
    const payloadPreferencia = {
      ...
      payer: dadosPagador, // Dados completos do pagador
    ...
    // Há duas formas de fazer Split Payments:
    // 1. Usando a API de Preferências (método mais simples)
    // 2. Usando a API de Payments (método mais complexo)
    // Vamos tentar os dois métodos:
    ...
    const payloadPagamento = {
      ...
      description: "Reserva de Espaço (TESTE)",
      external_reference: externalReference,
      ...
      capture: true,
      // API v1 usa application_fee em vez de marketplace_fee
      // e collector_id não está disponível diretamente
    };
